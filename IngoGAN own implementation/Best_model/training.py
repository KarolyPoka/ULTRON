...
# Folder to the tensorboard logs
...
class Training:
    ...
    # Funtion for categorical vector conversion to image generation
    ...
    def train(self, train_gen=True, train_disc=True, train_info=True, num_epochs=20, train_encoder=False):
        ...
        for epoch in range(self.epochs_done, self.loaded_epochs):
            ...
            if (train_encoder == False):
                for i, (imgs, _) in enumerate(self.dataloader):
                    ...
                    # # Accuracy of discriminator Acc=(TN+TP)/(TN+TP+FN+FP) -->depricated
            ...
            if(train_encoder == True):
                for i, (imgs, _) in enumerate(self.dataloader):
                    ...
                    pred_noise, cont_code = self.noise_predictor(real_imgs)

                    gen_imgs = self.generator(pred_noise, cont_code)
        ...
        # Saving to whole model -->depricated
    ...
    def plot(self, transform, path=None):
        ...
        # Plotting when there is no given image
        if path is None:
        ...
        # Plotting the variation of a given image
        else:
            img = Image.open(path)
            img_t = transform(img)
            ...
            # # #Image embedding
            # # ---------------
            # sampled_labels = np.random.randint(0, self.params["n_classes"], 1)
            # Zinit = FloatTensor(np.random.normal(0, 1, (1, self.params["latent_dim"])))
            # Cont = FloatTensor(np.random.uniform(-1, 1, (1, self.params["code_dim"])))
            # Zinit.requires_grad = True
            ...
            # # Download it before to the root folder from : "https://download.pytorch.org/models/vgg16-397923af.pth"
            # vgg16 = models.vgg16()
            # vgg16.load_state_dict(torch.load("vgg16-397923af.pth"))
            # # print(vgg16.classifier[6].out_features) # 1000
            # # Cut the feature network at the 24th layer.
            ...
            # print(extractor)
            #
            # optZ = torch.optim.Adam([Zinit, Cont], lr=0.01)
            # ms_ssim_loss = pytorch_msssim.MSSSIM()
            # l1 = nn.SmoothL1Loss()
            # inv_loss = []
            # EP = 17000
            #
            # xHAT = self.generator(Zinit, Cont)
            # for i in range(EP):
            #     optZ.zero_grad()
            #     xHAT = self.generator(Zinit, Cont)
            #     fake_feature = extractor(xHAT)
            #     real_feature = extractor(img)
            #     # print(xHAT.type())
            #     # print(img.type())
            #     # recLoss = F.mse_loss(xHAT, img)
            #     recLoss = l1(fake_feature, real_feature)
            #
            #     recLoss.backward()
            #     optZ.step()
            #     if i % 100 == 0:
            #         print("[Epoch %d/%d] [N loss: %f]" % (i, EP, recLoss.item()))
            # save_image(xHAT.data, "VGG_supervisored" + str(EP) + ".png", nrow=n_row, normalize=True)
            #
            # img_z = torch.cat([Zinit, ] * n_row ** 2)
            #
            # cont_code = torch.cat([Cont, ] * n_row ** 2)
            # static_sample = self.generator(img_z, cont_code)
            # save_image(static_sample.data, "finetuned_static_" + str(self.epochs_done) + ".png", nrow=n_row,
            #            normalize=True)
            #
            # zeros = np.zeros((n_row ** 2, 1))
            # c_varied = np.repeat(np.linspace(-1, 1, n_row)[:, np.newaxis], n_row, 0)
            ...
            # save_image(sample1.data, "finetuned_varied_c1_" + str(self.epochs_done) + ".png", nrow=n_row, normalize=True)
            # save_image(sample2.data, "finetuned_varied_c2_" + str(self.epochs_done) + ".png", nrow=n_row, normalize=True)
