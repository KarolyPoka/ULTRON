...
class discriminator(nn.Module):
    def __init__(self, latent_dim, n_classes, code_dim, img_size, channels):
        ...
        self.noise_layer = nn.Sequential(nn.Linear(256 * discriminator_size ** 2, 512 * discriminator_size ** 2),
                                         nn.LeakyReLU(0.1, inplace=True),
                                         nn.BatchNorm1d(512 * discriminator_size ** 2),
                                         nn.Linear(512 * discriminator_size ** 2, 512 * discriminator_size ** 2),
                                         nn.LeakyReLU(0.1, inplace=True),
                                         nn.BatchNorm1d(512 * discriminator_size ** 2),
                                         nn.Linear(512 * discriminator_size ** 2, latent_dim))
