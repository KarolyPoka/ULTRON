...
class generator(nn.Module):
    def __init__(self, latent_dim, n_classes, code_dim, img_size, channels):
        ...
        self.gen_layers = nn.Sequential(
            nn.ConvTranspose2d(input_dim, 448, 2, 1, bias=False),
            nn.BatchNorm2d(448),
            nn.ReLU(),
...
class discriminator(nn.Module):
    def __init__(self, latent_dim, n_classes, code_dim, img_size, channels):
        ...
        self.classifier_layer = nn.Sequential(nn.Linear(256, 1), nn.Sigmoid())
        self.categorical_layer = nn.Sequential(nn.Linear(256, n_classes), nn.Softmax())
        self.continuous_layer = nn.Sequential(nn.Linear(256, code_dim))
        self.noise_layer = nn.Sequential(nn.Linear(256, latent_dim))
