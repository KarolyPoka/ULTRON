...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for i, (imgs, labels) in enumerate(dataloader):
            ...
            static_z = Variable(FloatTensor(np.random.normal(0, 1, (batch_size, latent_dim))))
            static_class = to_categorical(np.random.randint(0, n_classes, batch_size), num_columns=n_classes)
            static_code = Variable(FloatTensor(np.random.uniform(-1, 1, (batch_size, code_dim))))
        ...
        bar.finish()
        print("[Epoch %d/%d] [D loss: %f] [G loss: %f] [info loss: %f]" % (epoch+1, num_epochs, d_loss.item(),
                                                                           g_loss.item(), info_loss.item()))



    plt.figure()
    plt.plot(range(num_epochs), discriminator_loss, "b-", label="discriminator_loss")
    plt.plot(range(num_epochs), generator_loss, "y-", label="generator_loss")
