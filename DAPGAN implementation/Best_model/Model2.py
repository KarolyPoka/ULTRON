...
class noise_predictor(nn.Module):
    def __init__(self, latent_dim, n_classes, code_dim, img_size, channels):
        ...
        self.n_pred_layers = Encoder()

        self.continuous_layer = nn.Sequential(nn.Linear(64, 128),
                                              nn.Linear(128, 256),
                                              nn.Linear(256, code_dim),
                                              nn.Tanh())

        self.noise_layer = nn.Sequential(nn.Linear(64, 128),
                                         nn.Linear(128, 256),
