...
if __name__ == "__main__":
    ...
    # Loss functions
    loss_funcs = {
        'classifier_loss':  nn.SmoothL1Loss(),
        'categorical_loss': nn.CrossEntropyLoss(),
        'continuous_loss': nn.SmoothL1Loss(),
        'noise_loss': nn.MSELoss(),
        'finetune_loss': nn.SmoothL1Loss()
    }
    ...
    if path.exists("DAPGAN_44.tar"):
        state = torch.load('DAPGAN_44.tar')
    ...
    # pretrained_disc = torch.load("Celeba_disc100_64")
    #
    # if cuda:
    #     pretrained_gen.cuda()
    ...
    # train_obj.train(num_epochs=1, train_encoder=True)
    # train_obj.plot(transform, path=IMG)
    # #train_obj.plot(transform, path=None)
    # #
    # train_obj.train(num_epochs=3, train_encoder=True)
    # train_obj.plot(transform, path=IMG)
    # #train_obj.plot(transform, path=None)
    # #
    # train_obj.train(num_epochs=5, train_encoder=True)
    # train_obj.plot(transform, path=IMG)
    # #train_obj.plot(transform, path=None)
    # #
    # train_obj.train(num_epochs=5, train_encoder=True)
    ...
    d_loss, g_loss, i_loss, disc_accuracy, c_cat_loss, c_con_loss, n_loss, pred_cat, pred_con, epochs_done = train_obj.train(num_epochs=0, train_encoder=False)
    train_obj.plot(transform, path=IMG)
    #train_obj.plot(transform, path=None)
    ...
    # ------------------------
    # plt.figure(2)
    # plt.title = "Predictor_after_" + str(len(N_loss)) + " epochs"
    # plt.plot(range(len(N_loss)), N_loss, "r-", label="N_loss")
    # plt.legend()
    # # plt.show()
    # plt.savefig("Predictor_after_ " + str(len(N_loss)) + " epochs" + ".png")
