...
class Training:
    ...
    def train(self, train_gen=True, train_disc=True, train_info=True, num_epochs=20, train_encoder=False):
        ...
        for epoch in range(self.epochs_done, self.loaded_epochs):
            ...
            if(train_encoder == True):
                for i, (imgs, _) in enumerate(self.dataloader):
                    ...
                    n_loss = 1 - self.finetune_loss(real_imgs, gen_imgs)
        ...
        torch.save(state, "DAPGAN_P_"+str(self.epochs_done)+".tar")
    ...
    def plot(self, transform, path=None):
        ...
        if path is None:
            ...
            # Zinit = torch.load("Zinit_girl50000")
            # Cat = torch.load("Cat_girl50000")
            # Cont = torch.load("Cont_girl50000")
            ...
            for i in range(EP):
                ...
                recLoss = 1 - ms_ssim_loss(xHAT, img)
            ...
            save_image(xHAT.data, "1_64_girl" + str(EP) + ".png", nrow=n_row, normalize=True)
            #save_image(img.data, "tony_64x64.png", nrow=n_row, normalize=True)

            #torch.save(Zinit, "Zinit_girl" + str(EP))

            #torch.save(Cont, "Cont_girl" + str(EP))
            ...
            save_image(static_sample.data, "SSIM64_static_" + str(self.epochs_done) + ".png", nrow=n_row, normalize=True)
            ...
            save_image(sample1.data, "SSIM64_varied_c1_" + str(self.epochs_done) + ".png", nrow=n_row, normalize=True)
            save_image(sample2.data, "SSIM64_varied_c2_" + str(self.epochs_done) + ".png", nrow=n_row, normalize=True)
