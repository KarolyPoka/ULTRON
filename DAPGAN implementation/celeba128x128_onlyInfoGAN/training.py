...
class Training:
    def __init__(self, G, D, N, losses, optimizers, dataloader, hyperparams):
        ...
        for epoch in range(num_epochs):
            ...
            if (train_encoder == False):
                for i, (imgs, _) in enumerate(self.dataloader):
                    ...
                    real_pred, _, _ = self.discriminator(disc_real_input)
                    ...
                    fake_pred, _, _ = self.discriminator(disc_fake_input)
            ...
            if(train_encoder == True):
                for i in range(len(self.dataloader)):
                    ...
                    pred_noise, pred_cat_c, pred_con_c = self.noise_predictor(gen_imgs)

                    n_loss = self.noise_loss(pred_noise, static_z)
                ...
                print("[Epoch %d/%d] [N loss: %f] [c_cat_loss: %f] [c_con_loss: %f]"
                      % (epoch + 1, num_epochs, (batch_N_loss / len(self.dataloader)),
